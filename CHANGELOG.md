...
- 資料品質的「CMDB 有但掃描未見」會反查目前資產主檔，帶出可編輯的資產識別。
...
- 資料品質匯入最近掃描報告前，會先以目前 CMDB 主檔 IP 重新覆核。
- 已存在於未退休資產的 IP 不再被舊掃描結果誤報為「掃描到但 CMDB 無」。
- 同一份品質報告會去除重複的舊掃描異常，讓真正需要處理的幽靈主機更清楚。
...
- 下線、待刪除、汰除資產預設不出現在日常清單，需明確勾選顯示或直接篩選狀態。
- 批次狀態更新後回到清單顯示成功、略過與目標狀態，讓操作結果可確認。
...
- 「尚未巡檢」空狀態加入開始巡檢目前範圍按鈕，巡檢 API 會帶入目前平台與系統範圍。
- JSON API、L1/L2/L3 帳號需求與臨時密碼說明收進進階區，主畫面只保留日常操作。
- 資產管理列表預設每頁改為 100 筆，降低大量資料查找時的翻頁成本。
...
- 未指定核心的系統改顯示在「未歸屬核心」，避免第一欄誤導。
- 拓撲系統下拉選單改以資產名稱 / 系統名稱為主，內部 `SYS-*` ID 只留在搜尋索引，不直接顯示給使用者。

## v1.0.3.65 - 2026-05-31 14:45 +08:00 - topology-full-asset-master-scan

- 核心影響圖補齊主機時改讀完整資產主檔，不再受資產清單頁每頁上限影響而少畫。
- 保留 v1.0.3.64 的 asset_seq 優先主機 key，讓同一系統下多台主機能穩定出現在第三欄。
- 草稿轉正式完成後改顯示成功筆數、仍留草稿筆數與略過原因，避免使用者按完後不知道是否完成。
...
- 資產同步到拓撲時保留 `system_id` 作為穩定 key，避免單純改顯示文字造成既有關聯斷裂。
- 資產管理搜尋、開門檢查搜尋與新增主機提示同步改成資產名稱 / 系統名稱，讓同一件事在各頁叫法一致。

## v1.0.3.61 - 2026-05-31 13:05 +08:00 - topology-searchable-selector

- 系統拓撲的核心影響與系統關聯選擇器新增可輸入篩選欄位，資料量大時不用只靠原生下拉尋找。
- 篩選可比對系統名稱、系統代號、owner、主機參照與描述，讓使用者用手上線索快速定位。
- 保留原本 `center`、`view`、`depth` 表單參數與節點點選流程，避免破壞既有拓撲連結與全螢幕操作。

## v1.0.3.60 - 2026-05-31 12:35 +08:00 - initial-draft-promote-all

- 草稿檢視新增建置期「全部草稿轉正式」入口，適合第一次匯入後一次把既有草稿正式化。
- 後端支援 `scope=all_drafts`，會抓目前全部草稿逐筆轉正式；資料完整者轉 `active`，不完整者略過並回報數量。
...
- 正式資產仍保留完整必填、連線與 OS 檢查；批次轉正式時仍會阻擋資料不完整的主機，避免垃圾資料直接進巡檢。
- 資產編輯頁只在正式資產顯示硬性必填星號與 HTML required，草稿階段改為先暫存、再逐步補齊。
...
- 上方全域搜尋改為分類資料結果頁，不再只當作功能捷徑或把使用者丟回資產列表。
- 同一個關鍵字可同時查正式資產、草稿 / 未納管、CMDB 資產池與功能入口。
- 結果列顯示類型、摘要、IP/Hostname/OS 與負責人資訊，方便用 IP、主機名、人員、AP 或軟體名稱快速定位。
- 若只命中功能入口且沒有資料結果，仍保留原本快速跳轉行為。

## v1.0.3.54 - 2026-05-31 09:30 +08:00 - cumulative-cmdb-3-44-to-3-53

- 改為累積修補包，補齊 v1.0.3.44 到 v1.0.3.53 的可部署 CMDB/資產管理修補。
- 修正現場可能出現「版號同為 v1.0.3.53，但功能不同」的問題；原因是先前 v1.0.3.53 為增量包，直升不同底版時可能漏掉 v1.0.3.51 的 `CMDB 資產池` 工具列入口。
- 累積帶入主機資料目錄權限防護、CMDB 資產池、分類編輯、空狀態診斷、整份治理匯入與主機儲存錯誤防護。
- 後續現場若不是逐版升級，請優先使用 v1.0.3.54 或更新的累積包，避免 UI 與服務檔版本不一致。

## v1.0.3.53 - 2026-05-30 09:15 +08:00 - cmdb-direct-governed-import

- CMDB Excel 匯入主流程改為直接 `整份治理匯入`，不再要求先按預檢。
- `只預檢，不寫入` 保留在進階匯入動作內，適合需要先看 sheet 分流的人使用。
- 移除 `1. 預檢` / `2. 匯入` 的步驟編號，避免使用者誤以為預檢是必經流程。

## v1.0.3.52 - 2026-05-29 15:00 +08:00 - cmdb-asset-pool-empty-diagnosis

- CMDB 資產池為 0 時新增空狀態診斷，說明主機 CSV / Excel 匯入會進資產清冊，不會自動進資產池。
- 空狀態顯示主機清冊總數、最近主機匯入、最近 Excel 預檢與最近資產池匯入，避免使用者以為資料消失。
- 補上下一步按鈕，可直接去資產清冊看已匯入主機，或回上方執行整份治理匯入。

## v1.0.3.51 - 2026-05-29 14:30 +08:00 - cmdb-asset-pool-menu-entry

- 資產管理頁工具列新增 `CMDB 資產池` 分類入口，不再把人員、軟體與資料資產藏在匯入精靈底部。
- `CMDB 資產池` 可直接進入全部、主機證據、資料資產、軟體 / AP、人員窗口與待分類。
- `更多操作` 選單新增資料維護、匯出 / 匯入、治理工具分類小標，讓按鈕變多時仍維持清楚。

## v1.0.3.50 - 2026-05-29 14:05 +08:00 - cmdb-asset-pool-tabs-edit

- CMDB 資產池新增分類檢視：全部、主機證據、資料資產、軟體 / AP、人員窗口、待分類。
- 資產池項目新增編輯頁，人員、軟體、資料資產可直接修正整理後欄位。
- 編輯保留原始 Excel `raw` 欄位，並要求填寫變更原因，避免來源證據遺失。
- 匯入精靈列表新增每列「編輯」動作，讓匯入後可立即整理。
