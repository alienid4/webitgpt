...
AUTO_NATIVE_REPOS="${AUTO_NATIVE_REPOS:-1}"
...
install_native_packages() {
  echo "Installing prerequisites from target host repositories."
...
native_podman_available() {
  command -v dnf >/dev/null 2>&1 || return 1
  dnf -q list podman >/dev/null 2>&1
}

ensure_native_runtime_if_available() {
  if [ "$SKIP_RPMS" = "1" ] || [ "$SKIP_MONGO" = "1" ] || command -v podman >/dev/null 2>&1; then
    return 0
  fi

  if [ "$AUTO_NATIVE_REPOS" != "1" ]; then
    return 0
  fi

  if grep -qi 'Red Hat Enterprise Linux.* 9\.6' /etc/redhat-release 2>/dev/null && native_podman_available; then
    echo "AUTO_NATIVE_REPOS=1; podman is available from RHEL 9.6 repositories."
    install_native_packages
  fi
}
...
wait_mongo() {
  if [ "$SKIP_MONGO" = "1" ]; then
    echo "SKIP_MONGO=1; skipping local MongoDB TCP readiness check."
    return 0
  fi

  if [ -n "${MONGO_URI:-}" ] && ! printf '%s' "$MONGO_URI" | grep -Eq 'localhost|127\.0\.0\.1'; then
    echo "MONGO_URI points to a non-local host; skipping local MongoDB TCP readiness check."
    return 0
  fi
