...
- `docs/20260527/v1.0.3.32_field-issue-intake-and-masking.md`

## Current Install Path

Use this after applying the
`v1.0.3.32_rhel96-native-podman-mongo-autofix-patch` to the extracted full
offline package.

The new patch auto-installs native RHEL 9.6 container runtime packages when
`podman` is missing but available from the target host repositories.

If diagnostics show `podman: command not found` while `dnf list podman` shows
`podman` as `Available Packages`, install the native RHEL 9.6 container runtime
packages first, then rerun the installer while skipping the local RPM bundle.

Recommended path after applying the new patch:
...
Manual fallback if the target host blocks automatic native package install:
...
Expected reason: MongoDB is started from the packaged container image by
`podman`. If `podman` is missing, the installer cannot load or start MongoDB,
and app bootstrap fails with `localhost:27017: [Errno 111] Connection refused`.
...
## Native Podman / Mongo Autofix Patch Apply Commands

Use this patch when RHEL 9.6 has native repositories available but the full
offline installer did not install `podman`, causing MongoDB to stay down.

```bash
cd /tmp/webitgpt_full_offline_1.0.3.32-api-key-verify-visibility_rhel-9-6_20260527081023
tar -xzf /tmp/v1.0.3.32_rhel96-native-podman-mongo-autofix-patch_20260527100043.tar.gz -C /tmp
bash /tmp/v1.0.3.32_rhel96-native-podman-mongo-autofix-patch/install_patch_rhel96_installer.sh "$(pwd)"
sudo bash INSTALL_ALL.sh
```

## Older Hotfix Patch Apply Commands

Use this only if the full offline package has already been extracted and the
older RHEL 9.6 installer hotfix patch has been downloaded.
...
- Native Podman / Mongo autofix patch: <https://github.com/alienid4/webitgpt/releases/download/v1.0.3.32/v1.0.3.32_rhel96-native-podman-mongo-autofix-patch_20260527100043.tar.gz>
- Native Podman / Mongo autofix SHA256: <https://github.com/alienid4/webitgpt/releases/download/v1.0.3.32/v1.0.3.32_rhel96-native-podman-mongo-autofix-patch_20260527100043.tar.gz.sha256>
