...
def test_superadmin_full_system_surfaces_exist():
    ...
    core_assignments = read("webapp/templates/core_assignments.html")
    assert "核心歸屬管理" in core_assignments
    assert "system_ids" in core_assignments
    assert "已合併" in core_assignments
    assert "duplicate_count" in read("webapp/services/dependency_service.py")
...
def test_asset_governance_status_admin_contracts_exist():
    ...
    for text in ["asset-table-compact", "序號", "資產狀態", "資產名稱 *", "Hostname", "asset-row-menu"]:
        assert text in hosts_template
    assert hosts_template.index('class="asset-col-row-number"') < hosts_template.index('class="asset-col-status"')
    assert '((data.get("page", 1) - 1) * data.get("page_size", 100)) + loop.index' in hosts_template
    for text in ["asset-col-row-number", "asset-row-number-cell", "asset-col-status", "asset-status-pill", "asset-status-active", "asset-status-draft", "asset-status-disabled", "asset-status-retired"]:
