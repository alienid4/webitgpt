...
def test_cmdb_relationship_overview_reads_all_host_pages(monkeypatch):
    from webapp.services import cmdb_relationship_service

    hosts = [
        {
            "hostname": f"app{i:03d}",
            "asset_name": "paged-system",
            "ip": f"10.0.0.{i}",
            "status": "active",
            "system_name": "paged-system",
            "owner": "ops",
            "open_ports": [{"port": "22"}],
        }
    ...
    def fake_list_hosts(**kwargs):
        page = int(kwargs.get("page") or 1)
        page_size = min(max(int(kwargs.get("page_size") or 25), 1), 100)
        start = (page - 1) * page_size
        return {
            "items": hosts[start : start + page_size],
            "total": len(hosts),
            "page": page,
            "page_size": page_size,
        }
    ...
    monkeypatch.setattr(cmdb_relationship_service.host_service, "list_hosts", fake_list_hosts)
    monkeypatch.setattr(cmdb_relationship_service, "get_collection", lambda _name: FakeCollection())

    overview = cmdb_relationship_service.cmdb_relationship_overview("paged")

    assert overview["summary"]["total"] == 205
    assert overview["summary"]["formal_count"] == 205
    assert overview["summary"]["system_row_count"] == 1
    assert overview["systems"][0]["host_count"] == 205
    assert overview["coverage"]["system"]["count"] == 205
...
def test_cmdb_relationship_overview_does_not_group_by_asset_group_name(monkeypatch):
    ...
    assert counts["待補系統關聯"] == 1
    assert "防火牆" not in counts
    assert "shared-infra-group" not in counts


def test_cmdb_relationship_overview_ignores_device_category_asset_names(monkeypatch):
    from webapp.services import cmdb_relationship_service

    hosts = [
        {
            "hostname": "generic-vm-1",
            "asset_name": "vmware_host",
            "device_type": "vmware_host",
            "group_name": "vmware_host",
            "ip": "10.0.0.1",
            "status": "active",
            "owner": "ops",
        },
        ...
        {
            "hostname": "explicit-vm-1",
            "system_name": "virtual-platform",
            "asset_name": "vmware_host",
            "device_type": "vmware_host",
            "ip": "10.0.0.3",
            "status": "active",
    ...
    counts = {row["display_name"]: row["host_count"] for row in overview["systems"]}
    classified_counts = {
        row["display_name"]: row["host_count"]
        for row in overview["systems"]
        if row["is_classified"]
    }

    assert classified_counts == {"tech-ocp": 1, "virtual-platform": 1}
    assert counts["待補系統關聯"] == 1
...
def test_asset_management_summary_uses_ingestion_governance_language():
    ...
    routes = read("webapp/routes/api_hosts.py")
    css = read("webapp/static/css/cathay.css")
    js = read("webapp/static/js/hosts.js")
    ...
    assert "assetBreakdownFilter" in hosts
    assert "assetBreakdownShowAll" in hosts
    assert "assetBreakdownFilterCount" in hosts
    assert 'id="assetBreakdownResults" hidden' in hosts
    assert "data-asset-breakdown-row" in hosts
    assert "data-filter-text" in hosts
    assert ".asset-breakdown-filter" in css
